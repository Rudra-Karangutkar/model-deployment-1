...
if not os.path.isfile(r'login.csv'):
    with open(r'login.csv', 'w', newline='') as file:
...
app.config['MAIL_USERNAME'] = 'projectbyme123@gmail.com'  # replace with your email
app.config['MAIL_PASSWORD'] = 'evpz rmvu ijxe iqod' 
...
def register():
    ...
    try:
        with open(r'login.csv', 'r') as file:
    ...
    if otp_code1[0] == str(verification_code):
        # Save data to CSV
        with open(r'login.csv', 'a', newline='') as file:
            writer = csv.writer(file)
